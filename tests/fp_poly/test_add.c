...
void test_error()
{
    ...
    assert (fp_poly_add(&res, NULL, q, NULL) == FP_POLY_E_POLYNOM_IS_NULL);
    assert (fp_poly_add(&res, p, NULL, NULL) == FP_POLY_E_POLYNOM_IS_NULL);
    mem = p->coeff;
    p->coeff = NULL;
    assert (fp_poly_add(&res, p, q, NULL) == FP_POLY_E_LIST_COEFFICIENT_IS_NULL);
    p->coeff = mem;
    mem_node = p->coeff->head;
    p->coeff->head = NULL;
    assert (fp_poly_add(&res, p, q, NULL) == FP_POLY_E_POLYNOM_MANIPULATION);
