...
int main()
{
    ...
    assert (list_destroy(NULL) == LIST_E_LIST_IS_NULL);
    assert (list_add_beginning(NULL, 0) == LIST_E_LIST_IS_NULL);
    assert (list_add_end(NULL, 0) == LIST_E_LIST_IS_NULL);
    assert (list_remove_node(NULL, NULL) == LIST_E_LIST_IS_NULL);
    assert (list_remove_node(NULL, 0) == LIST_E_LIST_IS_NULL);
    assert (list_remove_head(NULL) == LIST_E_LIST_IS_NULL);
    assert (list_remove_tail(NULL) == LIST_E_LIST_IS_NULL);
    assert (list_add_at(NULL, 0, 0) == LIST_E_LIST_IS_NULL);
    assert (list_print(NULL, NULL) == LIST_E_LIST_IS_NULL);
    assert (list_assert(NULL, (uint8_t []) {1}, 1) == LIST_E_LIST_IS_NULL);
    assert (list_create_from_array(NULL, 0) == NULL);
    assert (list_add_after(NULL, 0, NULL) == LIST_E_LIST_IS_NULL);
    assert (list_get_pos(NULL, NULL) == LIST_E_LIST_IS_NULL);
    assert (list_get_at_pos(NULL, 0) == NULL);

    // error cases: other
    list = list_init();
    assert (list_remove_coeff(list, 0) == LIST_E_LIST_MANIPULATION);
    assert (list_remove_node(list, NULL) == LIST_E_LIST_MANIPULATION);
    assert (list_remove_head(list) == LIST_E_LIST_MANIPULATION);
    assert (list_remove_tail(list) == LIST_E_LIST_MANIPULATION);
    assert (list_add_beginning(list, 1) == LIST_E_SUCCESS);
    assert (list_add_at(list, 2, 3) == LIST_E_LIST_MANIPULATION);
    assert (list_get_pos(list, NULL) == LIST_E_LIST_MANIPULATION);
    assert (list_remove_coeff(NULL, 0) == LIST_E_LIST_IS_NULL);
    assert (list_remove_coeff(list, 2) == LIST_E_LIST_MANIPULATION);
    assert (list_assert(list, NULL, 1) == LIST_E_LIST_MANIPULATION);
    assert (list_assert(list, (uint8_t []) {2}, 1) == LIST_E_LIST_MANIPULATION);
    assert (list_add_after(list, 0, NULL) == LIST_E_LIST_MANIPULATION);
    list_t *list2 = list_init();
    assert (list_add_beginning(list2, 1) == LIST_E_SUCCESS);
    assert (list_assert(list2, (uint8_t []) {1}, 2) == LIST_E_LIST_MANIPULATION);
    assert (list_get_pos(list2, list->head) == LIST_E_LIST_MANIPULATION);
    assert (list_remove_node(list, list2->head) == LIST_E_LIST_MANIPULATION);
    assert (list_remove_tail(list2) == LIST_E_LIST_MANIPULATION);
    ...
    assert (list_print(NULL, list) == LIST_E_FILE_DESCRIPTOR_IS_NULL);
