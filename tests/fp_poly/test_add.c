...
static void add_polynom(fp_poly_t *p, fp_poly_t *q, fp_field_t *f, size_t expected_index_coeff, uint8_t *expected_coeff, uint8_t len_expected_coeff)
{
    fp_poly_t *res;
...
static void hello_world_tests()
{
    ...
    add_polynom(p1, p2, NULL, 15, (uint8_t[]) {3, 2, 2, 7}, 4); // addition of p1 = 2 + 2*x + x^2 + 6*x^3 and p2 = 1 + x^2 + x^3 => res = 3 + 3*x + 2*x^2 + 7*x^3
    ...
    add_polynom(p1, p2, NULL, 479, (uint8_t[]) {2, 2, 1, 1, 1, 1, 1, 2}, 8); // addition of p1 = 1 + x + x^2 + x^3 + x^7 + x^8 and p2 = 1 + x + x^4 + x^6 + x^8 => res = 2 + 2*x + x^2 + x^3 + x^4 + x^6 + x^7 + 2*x^8
    assert (fp_poly_free(p2) == FP_POLY_E_SUCCESS);
    assert (fp_poly_free(p1) == FP_POLY_E_SUCCESS);
}
